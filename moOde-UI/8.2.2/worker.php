...
$mntstatus_state = $_SESSION['mount_status'] == 1 ? 'enable' : 'disable';
$mntstatus_trigger = $_SESSION['mount_status'] == 1 ? 'start' : 'stop';
// if ($_SESSION['mount_status'] == 1) {
sysCmd('systemctl ' . $mntstatus_state . ' mountstatus.service');
sysCmd('systemctl ' . $mntstatus_state . ' mountstatus.timer');
sysCmd('systemctl ' . $mntstatus_trigger . ' mountstatus.service');
sysCmd('systemctl ' . $mntstatus_trigger . ' mountstatus.timer');
// }

if ($_SESSION['mount_status'] == 0) {
	// LED states
...
function runQueuedJob() {
	...
	switch ($_SESSION['w_queue']) {
		...
		case 'hdmiport':
			...
			sysCmd($cmd . ' > /dev/null');
			break;
		case 'mntstatus': // Mount status
			if ($_SESSION['w_queueargs'] == '1') {
				sysCmd('echo none | sudo tee /sys/class/leds/led0/trigger > /dev/null 2>&1');
				sysCmd('echo 0 | sudo tee /sys/class/leds/led1/brightness > /dev/null 2>&1');
				sysCmd('echo none | sudo tee /sys/class/leds/led1/trigger > /dev/null 2>&1');
			}
			$mntstatus_state = $_SESSION['w_queueargs'] == '1' ? 'enable' : 'disable';
			$mntstatus_trigger = $_SESSION['w_queueargs'] == '1' ? 'start' : 'stop';
			sysCmd('systemctl ' . $mntstatus_state . ' mountstatus.service');
			sysCmd('systemctl ' . $mntstatus_state . ' mountstatus.timer');
			sysCmd('systemctl ' . $mntstatus_trigger . ' mountstatus.service');
			sysCmd('systemctl ' . $mntstatus_trigger . ' mountstatus.timer');
			if ($_SESSION['w_queueargs'] == 0) {
				...
				$led1_trigger = explode(',', $_SESSION['led_state'])[1] == '0' ? 'none' : 'default-on';
				...
				sysCmd('echo ' . $led1_trigger . ' | sudo tee /sys/class/leds/led1/trigger > /dev/null 2>&1');
			}
			break;
		case 'actled': // LED0
			if (substr($_SESSION['hdwrrev'], 0, 7) == 'Pi-Zero') {
